...
from torchcraft_pure.model.hierarchical.birnn_meta_controller import META_MAAC
...
agent0 = META_MAAC(name='meta_controller', state_dim=single_state_dim * agent_num, action_dim=enemy_num,
                   agent_num=10, training_agent_num=5,
                   hidden_layer_sizes=[16], actor_lr=1e-4, q_func_lr=1e-3, v_func_lr=1e-3, batch_norm=True,
...
agents = [agent1, agent1, agent1, agent1, agent1]
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            while not (done or env.is_goal_reached()):
                ...
                if len(agent1.replay_buffer) >= min_pool_size:
                    if not printed:
