        ...
        def actor_network(scope_name, s, reuse=False):
            with tf.name_scope(scope_name):
                with tf.variable_scope(scope_name, reuse=reuse):
                    x = s
                    x = tf.reshape(x, shape=[-1, 10, 20])
                    # x = make_fc('fc1', x, self.hidden_layer_sizes[0],
                    #             activation_fn=tf.nn.relu,
                    #             batch_norm=True
                    #             )

                    x = make_lstm_layer('lstm1', x, self.hidden_layer_sizes[0],
                                        layer_num=1, sequence_len=18, keep_prob=1.)
        ...
        def critic_network(scope_name, s, a, reuse=False):
            with tf.name_scope(scope_name):
                with tf.variable_scope(scope_name, reuse=reuse):
                    ...
                    x = tf.reshape(x, shape=[64, -1, 20])
                    x = make_lstm_layer('lstm-s', x, self.hidden_layer_sizes[0],
                                        layer_num=1, sequence_len=18, keep_prob=1.)
